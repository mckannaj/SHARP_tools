...
%
function [updAcc] = getUpdateAccN2_RF_1A2(N2UpdThirds, excludeSubjs)
    
    for row=1:height(N2UpdThirds)
        % check for NaNs
        if (isnan(N2UpdThirds.FirstThirdAcc(row)) == 1)
            N2UpdThirds.FirstThirdAcc(row) = 0;
            fprintf('getUpdateAccN2_RF_1A2: NaN in the firstThirdAcc for subj %d; substituting 0 so as not to break.\n', N2UpdThirds.Subject(row));
        end
        if (isnan(N2UpdThirds.LastThirdAcc(row)) == 1)
            N2UpdThirds.LastThirdAcc(row) = 0;
            fprintf('getUpdateAccN2_RF_1A2: NaN in the lastThirdAcc for subj %d; substituting 0 so as not to break.\n', N2UpdThirds.Subject(row));
        end
    ...
    if (excludeSubjs == true)
        updAcc = excludeSubjects_RF_1A2(updAcc);
    end
end
