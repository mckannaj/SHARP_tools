% Zephy McKanna
% getEFmeasures_RF_1A1()
% 3/8/15
%
% This function takes as input the "all" table created by formTables_RF_1A3().
% NOTE: several of the functions called by this function are 1A3 or 1A2 functions;
% these have been checked to also work with (cleaned/updated) 1A1 data. 
% The rest have been changed to 1A1 versions.
%
...
%
% Since SSRT can't be calculated from 1A-1 data (no SSD), we don't clean or
% use Inhibit trials.
%
...
% Inhibit SSRT can't be calculated from 1A1 data, as there is no StopSignal
% Delay (SSD) in our Inhibit cues for 1A1.
%
...
% (This would only happen if some subject didn't encounter all EFs somehow...?)
%
...
%       Z: NOTE THIS HASN'T BEEN TESTED!!!
%
...
%
function [EFmeasures] = getEFmeasures_RF_1A1(allDataTable1A1, dualTasks, printTable)
    fprintf('Making data table compatible with 1A-3 functions.\n');
    [allData] = makeAllTableCompatible_1A1(allDataTable1A1);
    ...
    if (dualTasks == false)
        [~, upd_N2Trials, ~, ~] = getCleanUpdateTrials_RF_1A3('neither', dualTasks, allData);
    else
        [~, upd_N2Trials, ~, ~] = getCleanUpdateTrials_dualTasks_RF_1A3('neither', false, dualTasks, allData);
    end
...
%}
    ...
    [sw_scAcc] = getSwitchCostAcc_RF_1A2(thirdsAcc_swSwitch, thirdsAcc_swRepetition, false);
    ...
    [sw_scRT] = getSwitchCostRT_RF_1A2(thirdsRT_swSwitch, thirdsRT_swRepetition, false);

    fprintf('Calculating Update score (for N=2).\n');
    [~, thirdsAcc_updN2] = getNewAccuracy_RF_1A3(upd_N2Trials, false);
    [updAcc] = getUpdateAccN2_RF_1A2(thirdsAcc_updN2, false);

    fprintf('SSRT for Inhibit cannot be calculated for 1A-1.\n');
%    [SSRTtable] = getSSRTthirds_RF_1A2(inh_goTrials, inh_inhibitTrials, excludeSubjs);
    ...
    if (strcmpi('', printTable) == 0) % there's something in printThirds
        ...
        elseif (printTable == true) % do a default filename
            fileName = getFileNameForThisOS('getSingleEFmeasures_RF_1A1-output.csv', 'IntResults');
