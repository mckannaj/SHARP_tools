% Zephy McKanna
% putNewColsIntoDataTable_2B()
% 7/24/17
% 
% This function takes the "all" data matrix (or any subset of trials) made
% by the formTables_RF_1A3() function. Note that it expects 2B data; for
% 2A or 1B or 1A-3 data, use the appropriately named function.
%
...
% 5 = OcularInspection
%
% HypTesting  - zNOTE: should probably add a number to indicate that this task has been seen by this participant before (e.g., -1) 
%  Flag to indicate whether this shift involves abductive reasoning (that
%  is, is the entire task spelled out in the instructions, or must the
%  participant infer some sort of rule in order to complete the task?)
% 0 = There is no abductive reasoning in this task
% 1 = There is some aspect of abductive reasoning in this task (at least the first time it's seen)
%
% CatRule
%  Numerical code for the categorization rule in this shift:
% 0 = This is not a categorization shift
% 1 = Single Dimension (ID)
% 2 = AND
% 3 = OR
% 4 = xOR
% 5 = ShepardV
%
% SessionNum
%  This is included merely for legacy compatibility. The information is
%  now contained in the Visit column, but we make it numeric here.
% if Visit = 'trainingX', then SessionNum = X.
% 
% RfTNumber
%  This is included for compatibility with past and perhaps present RF_SUM
%  files. Equal to SessionNum.
%
function [newDataTable] = putNewColsIntoDataTable_2B(oldDataTable)
    if (nargin ~= nargin('putNewColsIntoDataTable_2B'))
        error('zError: putNewColsIntoDataTable_2B expects %d inputs, but received %d. Please update any calling code.\n', nargin('putNewColsIntoDataTable_2B'), nargin);
    end
    
    fprintf('zNote: not analyzing shift 4b T:2b - For exceptional testers; find out what this is based on!\n');
    warning('putNewColsIntoDataTable_2B has not incorporated the new Inhibit shift info, so Inhibit analyses will likely be wrong!');
    ...
    HypTesting = zeros(allRows,1); % 0 = this is not a hypothesis testing task
    CatRule = zeros(allRows,1); % 0 = this is not a categorization task
    SessionNum = zeros(allRows,1) - 1; % -1 = error (sanity check)
    RFversion = zeros(allRows,1) - 1; % -1 = error (sanity check)
    ...
    for row = 1:allRows % loop through all the trials %!!! ZEPHY: IS THERE NO BETTER WAY TO DO THIS? LOOPS ARE REALLY SLOW!
        ...
        elseif (oldDataTable.CumShiftNum(row) ~= oldDataTable.CumShiftNum(row-1))
        ...
        % translate the Visit into a SessionNum
        visit = oldDataTable.Visit{row};
        switch visit
            case 'training01'
                SessionNum(row) = 1;
            case 'training02'
            ...
            case 'training11'
                SessionNum(row) = 11;
            otherwise
                warning('putNewColsIntoDataTable_1B: unexpected visit: %s\n', visit);
        end
        
        version = oldDataTable.Training{row};
        switch version
            ...
            case 'robot_factory'
                RFversion(row) = 2;
            otherwise
                warning('putNewColsIntoDataTable_1B: unexpected RF version number: %s\n', version);
        end
...
% not in 1A3 or 1B            case 'Limb Management (I)'
            ...
            case {'Eye Inspection (U/I)', 'Head and Shoulders (U/I)', 'Chest Digitizing (U/I)', 'Face Numbering (U/I)', ...
                    'Cranial Equilibrium (U/I)', 'Arm Matching (U/I)', 'Brain Deficiencies (U/I)', 'Terrestrial Tiering (U/I)'}
...
% not in 1A3 or 1B            case 'Avoidance Training (S/I)'
            case {'Part ID Gleaning (U/S)', 'Eye Scrobbling (U/S)', 'Cross Checking (U/S)', ...
                    'Naturalized Decommissioning (U/S)', 'Aviation Aspectizing (U/S)', ...
                    'Intent Estimation (U/S)', 'Advanced Categorization (U/S)'}
                numOfEFsInThisShift(row) = 2;
                % N-back
...
% not in 1A3 or 1B            case 'Advanced Categorization (U/S)'
            case {'Eye Symmetrizing (U)', 'Verbal Repetition (U)', 'Localized Vision (U)', 'Spatial Troning (U)'}
                numOfEFsInThisShift(row) = 1;
...
% not in 1A3 or 1B            case 'Spatial Troning (U)'
            ...
            case  {'Eye Intermediation (I)', 'Serpentinian Exclusion (I)', 'Complex Biodigitizing (I)'}
                numOfEFsInThisShift(row) = 1;
                % Inhibit and Logic(xOR)
                if (strcmpi(trial.NextState, 'INHIBIT') == 1)
                    LogicXorFlag(row) = 1; % yes Inhibit, yes xOR
                    InhibitFlag(row) = 1; % this was an Inhibit trial, and they would've responded otherwise (true for all Inhibit trials now)                    
                else 
                    InhibitFlag(row) = 0; % there was no Inhibit cue on this trial
            ...
            case {'Sensor Peripheralizing (S)', 'Emotionalizing Literature (S)', 'Area Allocation (S)', 'Conveyance Arithmetic (S)'}
                numOfEFsInThisShift(row) = 1;
                % Switch
                if (canCheckPrevRow == 0) % we can not use the last row
                    SwitchFlag(row) = 0; % there is no way to determine if this was a switch or repetition trial
            ...
            case {'Digit Arrangement (U/I)', 'PI Memorization (U/I)', 'Language Crunching (U/I)', 'PIN Defraudment (U/I)'}
                numOfEFsInThisShift(row) = 2;
                % N-back
                if (strcmpi(trial.ExpResp, 'LEFT') == 1)
                    NbackFlag(row) = 1; % Should respond to n-back
...
% not in 1A3 or 1B            case 'PIN Defraudment (U/I)'
            case {'Ocular Orientation (S/I)', 'Cartography Catharsis (S/I)', 'Generational Gapping  (S/I)'}
                numOfEFsInThisShift(row) = 2;
                % Inhibit
                if (strcmpi(trial.NextState, 'INHIBIT') == 1)
                    InhibitFlag(row) = 3; % this was an Inhibit trial, but the "cue" was just the original stimulus
...
% not in 1A3 or 1B            case 'Generational Gapping  (S/I)'
            case {'Limb Chromatizing (U/S)', 'Genus Logistics (U/S)', 'Organizational Balancing (U/S)', ...
                    'Organism Grading (U/S)', 'Recognition Profiling (U/S)'}
                numOfEFsInThisShift(row) = 2;
                % Switch
                if (canCheckPrevRow == 0) % we can not use the last row
                    SwitchFlag(row) = 0; % there is no way to determine if this was a switch or repetition trial
...
% only 1306            case '4a Update Switch V1 - 2 (U/S)'
            case {'Biological Itemizing (U/S)', 'Reptile Plotting (U/S)', 'Word/Number Partitioning (U/S)'}
                ...
                if ((InhibitFlag(row) == 3) || (SwitchFlag(row) == 1))
                    NbackFlag(row) = 1; % if inhibit or switch, then n-back
                else
...
% only 1306            case '4a Update Switch V2 - 2 (U/S)'
            ...
            case 'Mammalian Maneuvering (U/I)'
                numOfEFsInThisShift(row) = 3;
                % N-back
                if ((strcmpi(trial.NextState, 'TASK_1_LEFT_MATCH') == 1) || (strcmpi(trial.NextState, 'TASK_1_RIGHT_MATCH') == 1) || ...
                        (strcmpi(trial.NextState, 'TASK_2_LEFT_MATCH') == 1) || (strcmpi(trial.NextState, 'TASK_2_RIGHT_MATCH') == 1))
            ...
            case {'4b T:1a - rm Prrtzng', '4b T:1b - O_gan Failure?'} % based on Arm Prioritizing
                ...
                HypTesting(row) = 1; % this is a 4b shift, so it has some aspects of hypothesis testing
            ...
            case '4b T:3b - Extreme Categorizing'
                ...
                LogicXorFlag(row) = 1; % all trials are xOR trials
                HypTesting(row) = 1; % this is a 4b shift, so it has some aspects of hypothesis testing
            case {'Simple Allocating', 'Compliance Distributing', 'Basic Organizing'} % ID tasks
                HypTesting(row) = 1; % this is a Categorization shift, so it has some aspects of hypothesis testing
                CatRule(row) = 1; % single-dimension (ID)
            case {'Sort Grouping', 'Logistical Analyzing', 'Mid-level Syngergizing'} % AND tasks
                HypTesting(row) = 1; % this is a Categorization shift, so it has some aspects of hypothesis testing
                CatRule(row) = 2; % AND
            case {'Advanced Allocating', 'Product Paradigming', 'Orchestrating Compliance', 'Input Processing'} % OR tasks
                HypTesting(row) = 1; % this is a Categorization shift, so it has some aspects of hypothesis testing
                CatRule(row) = 3; % OR
            case {'Refractilating Reticulations', 'Classification Sensing', 'Defraggling', 'Assemblage Fabrication'} % xOR
                HypTesting(row) = 1; % this is a Categorization shift, so it has some aspects of hypothesis testing
                CatRule(row) = 4; % xOR
            case {'Mega Clumpifying', 'Maximum Fingling', 'Complex Codifying', 'Intense Glombing'} % Shepard V
                HypTesting(row) = 1; % this is a Categorization shift, so it has some aspects of hypothesis testing
                CatRule(row) = 5; % Shepard V
            otherwise
    ...
    newDataTable.Problem = newDataTable.Trial + 1; % Legacy called Trial "Problem"; also, Trial is currently 0-based for some reason, whereas everything else is 1-based
    newDataTable.HypTesting = HypTesting;
    newDataTable.CatRule = CatRule;
    newDataTable.SessionNum = SessionNum;
    newDataTable.RFversion = RFversion;
    newDataTable.RfTNumber = SessionNum; % note: not a typo; these are identical, both just included for legacy code
