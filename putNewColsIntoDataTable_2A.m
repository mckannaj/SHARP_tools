% Zephy McKanna
% putNewColsIntoDataTable_2A()
% 10/24/16
% 
% This function takes the "all" data matrix (or any subset of trials) made
% by the formTables_RF_1A3() function. Note that it expects 2A data; for
% 1B or 1A-3 data, use the appropriately named function.
%
...
%
function [newDataTable] = putNewColsIntoDataTable_2A(oldDataTable)
...
% not in 1A3 or 1B            case 'Complex Biodigitizing (I)'
            ...
            case {'Digit Arrangment (U/I)', 'PI Memorization (U/I)', 'Language Crunching (U/I)', 'PIN Defraudment (U/I)'}
                numOfEFsInThisShift(row) = 2;
                % N-back
                if (strcmpi(trial.ExpResp, 'LEFT') == 1)
                    NbackFlag(row) = 1; % Should respond to n-back
