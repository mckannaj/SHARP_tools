% Zephy McKanna
% putNewColsIntoDataTable_1A3()
% 5/25/15
% 
% This function takes the "all" data matrix (or any subset of trials) made
% by the formTables_RF_1A3() function. Note that it expects 1A-3 data; for
% 1B data, use putNewColsIntoDataTable_1B().
%
...
%  Otherwise (InhibitFlag = 1), this equals SSD ("CurrentInhDelta")
%
% SwitchFlag
...
% 5 = OcularInspection
%
function [newDataTable] = putNewColsIntoDataTable_1A3(oldDataTable)
    if (nargin ~= nargin('putNewColsIntoDataTable_1A3'))
        error('zError: putNewColsIntoDataTable_1A3 expects %d inputs, but received %d. Please update any calling code.\n', nargin('putNewColsIntoDataTable_1A3'), nargin);
    end

    [CheckFor1306, CheckFor1B] = getShiftsNotSeenIn1A3(oldDataTable);
    if (height(CheckFor1B) > 0)
        error('putNewColsIntoDataTable_1A3: This function should be used for 1A-3 data; use a different one for 1B data.\n');
    elseif (height(CheckFor1306) > 0)
        fprintf('putNewColsIntoDataTable_1A3: This function will not work properly for data from subject 1306. Excluding these shifts from output.\n');
    ...
    for row = 1:allRows % loop through all the trials %!!! ZEPHY: IS THERE NO BETTER WAY TO DO THIS? LOOPS ARE REALLY SLOW!
        ...
        switch shift
            ...
            case {'Symbolic Matching (U)', 'Organism Culling (U)', 'Count Quantizing (U)'}
                numOfEFsInThisShift(row) = 1;
                % N-back
                if (strcmpi(trial.NextState, 'ACTION') == 1)
                    NbackFlag(row) = 1; % Should respond to n-back
...
% not in 1A3            case 'Memory Management(U)'
% not in 1A3            case 'Snake Identification (I)'
...
% not in 1A3 or 1B            case 'Limb Management (I)'
            case {'Green Synergizing (I)', 'Fault Limitations (I)'}
                numOfEFsInThisShift(row) = 1;
                % Inhibit
...
% not in 1A3            case 'Stereoizing (I)'
% not in 1A3            case 'Emotional Imprinting (I)'
% not in 1A3            case 'Map Orgling (I)'
            case {'Special Orders (S)', 'Monocular Inspection (S)', 'Head Hueing (S)', 'Fracture Mitigation (S)', ...
                    ...
                    'Chestial Numbering (S)', 'Designation Tinting (S)'}
                numOfEFsInThisShift(row) = 1;
                % Switch
                ...
                elseif (any(~cellfun('isempty',strfind(prevTrial.NextState, 'TASK_2'))) == 0) % previous trial was TASK_2
                    ...
                    frpintf('putNewColsIntoDataTable_1A3: Some odd Switch condition - subj=%d, shift=%s, prevTrial=%s, thisTrial=%s.\n', trial.Subject, trial.Shift, prevTrial.NextState, trial.NextState);
                    SwitchFlag(row) = 0; % there is no way to determine if this was a switch or repetition trial
                end
% not in 1A3            case 'Job Differentiation (S)'
...
% only 1306            case 'Serpentinian Exclusion (I)'
% not in 1A3 or 1B            case 'Complex Biodigitizing (I)'
            ...
            case {'Head Heuristics (S)', 'Avionic Sequentializing (S)'}
                numOfEFsInThisShift(row) = 1;
                % Switch
                if (canCheckPrevRow == 0) % we can not use the last row
                    SwitchFlag(row) = 0; % there is no way to determine if this was a switch or repetition trial
...
% not in 1A3 or 1B            case 'Conveyance Arithmetic (S)'
...
% not in 1A3 or 1B            case 'Word/Number Partitioning (U/S)'
            ...
            case '4b T:3a - Specializations' % based on Verbal Repetition
                ...
                NbackFlag(row) = 1; % all of these trials are N-back
                LogicXorFlag(row) = 1; % all of these trials are xOR
% not in 1A3            case '4b T:3c - Penultimations'
...
% not in 1A3            case '4b T:3b - Extreme Categorizing'
            otherwise
                warning('putNewColsIntoDataTable_1A3: unexpected shift: %s\n', shift);
        end
        ...
        if (InhibitFlag(row) == 1)
            InhibitDifficulty(row) = trial.CurrentInhDelta;
        end
        if (NbackFlag(row) >= 0)
