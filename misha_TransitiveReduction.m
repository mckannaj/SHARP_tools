...
%% TransitiveReduction(adjmat)
% Compute a transitive reduction of a DAG (directed ACYCLIC graph) 
% The DAG has to be specified in adjacency matrix form, 
% such that G(i,j)=1 <=>  j is ancestor (parent, predcessor) 
% j -> i is an edge from j to i   
% The algorithm: 
% 1. computes a topological order 
...
function red_adjmat = misha_TransitiveReduction(adjmat)
